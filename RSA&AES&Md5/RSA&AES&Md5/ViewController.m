...
- (void)viewDidLoad {
    ...
    NSString *pwd = @"123456";
    //1，md5
    //可以直接拿散列后的散列值去https://www.cmd5.com/ 查询，比较简单的散列值是直接能在这个网站看到原文的。
    NSLog(@"散列:%@",pwd.md5String);
    //16位和32位的区别，16位实际上是从32位字符串中，取中间的第 9 位到第 24 位的部分，所以在iOS中就是NSMakeRange(8, 16)。
//    NSLog(@"小写16:%@",pwd.md5StringForLower16Bate);
//    NSLog(@"大写16:%@",pwd.md5StringForUpper16Bate);

    //加盐,一旦加盐后，在cmd5网站就不容易查询到原文了。所以可以不能就单纯进行一次md5就完事了，可以进行多次md5,或者加盐。
    NSString  *salt = @"zdfsdfer34cfaz";
    NSLog(@"加盐:%@",[[pwd stringByAppendingString:salt] md5String]);

    //2,sha1/256/512
    NSLog(@"sha1:%@",pwd.shar1);

    //3,hmac,hmac散列后的散列值也是不容易在破解的。
    NSLog(@"hmac散列：%@",[pwd hmacMD5StringWithKey:@"kkkk"]);
